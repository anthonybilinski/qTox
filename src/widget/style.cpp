...
std::map<std::pair<QString, QFont>, std::weak_ptr<const QString>> Style::stylesheetsCache;

// caller must hold on to the returned shared_ptr as long as they use the stylesheet
std::shared_ptr<const QString> Style::getStylesheet(const QString& filename, QFont baseFont)
{
    const std::pair<const QString, QFont> cacheKey(filename, baseFont);
    ...
    if (it != stylesheetsCache.end())
    {
        if (auto val = it->second.lock()) {
            ...
            // miss, cache expired
            std::shared_ptr<const QString> newVal;
...
std::shared_ptr<const QString> Style::resolve(const QString& filename, QFont baseFont)
{
    QFile file{filename};
